...
* This involves assistance from other DNS servers to help resolve the hostname.
...
* For the example hostname "drive.google.com", our DNS server will ask the
  top-level ".com" DNS server which will forward to the "google.com" DNS server
  and so on, until the authoritative DNS server is found.
